        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (player != null && client.interactionManager != null) {
                if (dontParryKey.wasPressed()) {
                    ...
                    if (!FrycParry.config.client.holdDontUseParryKey)
                        player.sendMessage(Text.of("Prevent using parry: " + dontParryKeyPressed), true);
                }
                ...
                if (parrykey.isPressed() || (rightClick && client.options.useKey.isPressed())) {
                                ...
                                if (ClientParryHelper.canParry(player)) { // <-- checking client sided config
                                    if (ParryHelper.canParryWithoutShield(player)) { // <-- checking server sided config
                                        if (!player.getItemCooldownManager().isCoolingDown(player.getMainHandStack().getItem())) {
                                            if (((ParryItem) player.getMainHandStack().getItem()).frycparry_getUseParryAction(player.getMainHandStack()) == UseAction.BLOCK) {
                                                ((ParryInteraction) client.interactionManager).frycparry_interactItemParry(client.player, Hand.MAIN_HAND);
                                            }
                                        }
                                    }
                                }
                    ...
                    if (!player.isUsingItem()) {
                        if (bl) {
                            ((CanBlock) player).frycparry_setBlockingDataToFalse();
                            ((CanBlock) player).frycparry_setParryDataToFalse();
                            bl = false;
                        }
                    } else if (((CanBlock) player).frycparry_getBlockingDataValue()) {
                        ((ParryInteraction) client.interactionManager).frycparry_stopUsingItemParry(player); // <---- it sends STOP_BLOCKING_ID packet
                    }
                }
            }
        });
