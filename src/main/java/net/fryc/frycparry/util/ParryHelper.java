...
import net.fryc.frycparry.damage.ModDamageSources;
...
public class ParryHelper {
    ...
    public static boolean blockingFullyNegatesDamage(DamageSource source, ItemStack stack, LivingEntity user) {
        if (source.isExplosive()) {
            ...
            return explosionCanBeBlocked(parryItem) && explosionWasSuccessfullyBlocked(parryItem, user) && parryItem.frycparry_getParryAttributes().getExplosionDamageTakenAfterBlock() <= 0f;
        ...
        if (stack.getItem() instanceof ParryItem parryItem) {
            if (source.isProjectile()) {
                return parryItem.frycparry_getParryAttributes().getProjectileDamageTakenAfterBlock() <= 0f;
            }
            return parryItem.frycparry_getParryAttributes().getMeleeDamageTakenAfterBlock() <= 0f;
        }
        return true;
    }

    public static boolean explosionCanBeBlocked(ParryItem parryItem) {
        return parryItem.frycparry_getParryAttributes().getExplosionBlockDelay() > -1;
    }

    public static boolean explosionWasSuccessfullyBlocked(ParryItem parryItem, LivingEntity user) {
        return parryItem.frycparry_getParryAttributes().getMaxUseTimeParry() - user.getItemUseTimeLeft() > parryItem.frycparry_getParryAttributes().getExplosionBlockDelay();
    }
    ...
    public static boolean attackWasParried(DamageSource source, ItemStack stack, LivingEntity user) {
        ...
        if (isItemParryEnabled(stack)) {
            int maxUseTime = ((ParryItem) user.getActiveItem().getItem()).frycparry_getParryAttributes().getMaxUseTimeParry();
            int parryTicks = ((ParryItem) stack.getItem()).frycparry_getParryAttributes().getParryTicks();
            parryTicks += Math.abs(((ParryItem) stack.getItem()).frycparry_getParryAttributes().getBlockDelay() - ModEnchantments.getReflexEnchantment(user));// negative block delay increases parry ticks
    ...
    public static boolean isItemParryDisabledWithConfig(World world, ItemStack stack) {
        ...
        if (world.isClient()) {
            if (item instanceof SwordItem)
                return !ConfigHelper.enableBlockingWithSword && !stack.isIn(ModItemTags.ITEMS_CAN_PARRY);
            if (item instanceof AxeItem)
                return !ConfigHelper.enableBlockingWithAxe && !stack.isIn(ModItemTags.ITEMS_CAN_PARRY);
            if (item instanceof PickaxeItem)
                return !ConfigHelper.enableBlockingWithPickaxe && !stack.isIn(ModItemTags.ITEMS_CAN_PARRY);
            if (item instanceof ShovelItem)
                return !ConfigHelper.enableBlockingWithShovel && !stack.isIn(ModItemTags.ITEMS_CAN_PARRY);
            if (item instanceof HoeItem)
                return !ConfigHelper.enableBlockingWithHoe && !stack.isIn(ModItemTags.ITEMS_CAN_PARRY);
    ...
    public static boolean isItemParryDisabledWithConfig(ItemStack stack) {
        Item item = stack.getItem();
        if (item instanceof SwordItem)
            return !FrycParry.config.sword.enableBlockingWithSword && !stack.isIn(ModItemTags.ITEMS_CAN_PARRY);
        if (item instanceof AxeItem)
            return !FrycParry.config.axe.enableBlockingWithAxe && !stack.isIn(ModItemTags.ITEMS_CAN_PARRY);
        if (item instanceof PickaxeItem)
            return !FrycParry.config.pickaxe.enableBlockingWithPickaxe && !stack.isIn(ModItemTags.ITEMS_CAN_PARRY);
        if (item instanceof ShovelItem)
            return !FrycParry.config.shovel.enableBlockingWithShovel && !stack.isIn(ModItemTags.ITEMS_CAN_PARRY);
        if (item instanceof HoeItem)
            return !FrycParry.config.hoe.enableBlockingWithHoe && !stack.isIn(ModItemTags.ITEMS_CAN_PARRY);
    ...
    public static void applyParryEffects(LivingEntity user, LivingEntity attacker) {
        ...
        ParryAttributes parryAttributes = ((ParryItem) user.getActiveItem().getItem()).frycparry_getParryAttributes();
    ...
    public static void applyCounterattackEffects(PlayerEntity user, LivingEntity attacker) {
        ...
        if (counterattackEnchantmentLevel > 0) {
            ...
            if (user.getOffHandStack().isEmpty()) {
                ctrattack_damage *= (counterattackEnchantmentLevel * 0.2) + 0.1;
            } else {
                ctrattack_damage *= (counterattackEnchantmentLevel * 0.1) + 0.1;
            }

            attacker.damage(ModDamageSources.COUNTERATTACK, (float) ctrattack_damage);
        }
    }
    ...
    public static int getParryCooldown(PlayerEntity player, Item item) {
        float cooldown;
        if (((CanBlock) player).frycparry_hasParriedRecently()) {
            cooldown = ((ParryItem) item).frycparry_getParryAttributes().getCooldownAfterParryAction();
        } else {
            cooldown = ((ParryItem) item).frycparry_getParryAttributes().getCooldownAfterInterruptingBlockAction();
        }
